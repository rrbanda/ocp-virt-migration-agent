...
class TestMigrationSafetyCallback:
    ...
    @pytest.mark.asyncio
    async def test_non_migration_tool_passes_through(self, _other_tool, _tool_context):
        result = await migration_safety_callback(_other_tool, {"namespace": "ns"}, _tool_context)
        assert result is None

    @pytest.mark.asyncio
    @patch.dict(os.environ, {"MIGRATION_DRY_RUN": "true"})
    async def test_dry_run_blocks_migration(self, _mock_tool, _tool_context):
        ...
        result = await cb(_mock_tool, {"namespace": "ns", "vm_name": "vm1"}, _tool_context)
    ...
    async def test_normal_mode_allows_migration(self, _mock_tool, _tool_context):
        from importlib import reload

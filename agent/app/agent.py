...
_SINGLE_INSTRUCTION = (
    ...
    "resource `samples/pre-migration-playbook-output.txt`\n"
    ...
    "- Playbook YAML available at `samples/pre-migration-playbook.yml` and "
    "`samples/post-migration-playbook.yml`\n\n"
...
_COORDINATOR_INSTRUCTION = (
    ...
    "`samples/pre-migration-playbook-output.txt` for pre-migration sample output\n"
    "- Use `load_skill_resource` with skill `post-migration-validator` and resource "
    "`samples/post-migration-playbook-output.txt` for post-migration sample output\n"
    "- The sample playbook YAML is also available at `samples/pre-migration-playbook.yml` "
